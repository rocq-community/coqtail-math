...
intros An r Pr r0 r0_ub.
 ...
 assert (r'_bd : Rabs ((a + r) / 2) < r).
  ...
  apply Rmult_lt_compat_r ; [apply Rinv_0_lt_compat ; auto with *|] ;
  apply Rplus_lt_compat_r ; rewrite Rabs_right in a_bd ; auto with *.
...
intros An z l Hzl.
 ...
 assert (H1 : forall n :  nat, (n >= S N)%nat -> gt_norm_pser An z n
  ...
  apply HN ; auto with *.
   ...
   apply Rle_trans with (Rmax 2 (Cnorm (An 0%nat) + 1)) ; [apply H1 | apply RmaxLess2] ; auto with *.
Qed.
...
intros An r rho r' r'_bd.
 ...
 rewrite r'_lb ; rewrite IRC_pow_compat, pow_i ; [| auto with *] ;
 ...
 reflexivity.
  intro Hf ; rewrite Hf, Rabs_R0 in Rabsr_pos ; elim (Rlt_irrefl _ Rabsr_pos).
  rewrite Cnorm_IRC_Rabs ; left ; apply HN ; auto with *.
...
Proof.
intros An Pr z ; unfold sum_derive ; apply weaksum_r_derive_sums ; auto with *.
Qed.
...
assert (cv : forall z : C, Boule 0 r' z ->  {l : C |  Cseq_cv (fun N : nat =>
 ...
 assert (H : n = S (pred n)) by (auto with *) ; rewrite H ; clear H ; simpl.
 unfold CFseq_cvu, SFL_interv, CFpartial_sum in HN.
 assert (predn_lb : (N <= pred n)%nat) by auto with *.
...
intros An Pr z eps eps_pos. 
 assert (H : 0 <= Cnorm z < Cnorm z + 1).
  split ; [apply Cnorm_pos |] ; auto with *.
